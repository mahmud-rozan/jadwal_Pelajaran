<?php
    ...
    $jumlah_status = mysqli_query($db, "select count(*) as total from semester where status='1'");
    ...
    if($jumlah_status['total'] > 0){
        if($status == 1){
            mysqli_query($db, "update semester set status='0' where id_semester='$id_semester'");
        }else{
        ...
        $status = $status == 0 ? 1 : 0;
        mysqli_query($db, "update semester set status='$status' where id_semester='$id_semester'");
    }
